...
test('Meta verification returns its challenge only for the configured verify token', async () => {
...
test('Meta POST persists only an authentic normalized event and returns duplicate safely', async () => {
  ...
  const pool = { async query(sql) {
    if (sql.includes('insert into webhook_events')) return { rowCount: ++inserts === 1 ? 1 : 0, rows: [] };
